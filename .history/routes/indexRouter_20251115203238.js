...
router.get('/cart',isLoggedIn, async function(req,res){
    let user = await userModel.findOne({email : req.user.email}).populate('cart').populate('cart.product');//if you dont populate cart image 
...
router.get('/quantity/:productid',isLoggedIn, async function(req,res){
    let user = await userModel.findOne({email : req.user.email});


    await userModel.updateOne({_id : req.params.productid},
        {$inc : {quantity : 1}}
);
    res.redirect("/cart");
})
