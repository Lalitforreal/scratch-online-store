...
router.get('/create-order', isLoggedIn, function(req,res){
  res.redirect('/cart');
})
...
//   let user = req.user; this will not contain teh updated cart so do this instead
  ...
  try{
    ...
    return res.redirect(`/order-summary/${newOrder._id}`);
...
//order summary 
router.get('/order-summary/:orderid', async function(req,res){
    const order = await orderModel.findOne({_id : req.params.orderid})
    .populate("items.product");
    res.render("order-summary", {orderItems : order.items});
})
