...
router.get('/cart',isLoggedIn, async function(req,res){
    ...
    let user = await userModel.findOne({_id : req.user._id}).populate('cart.product');//if you dont populate cart image 
    //will just be obj id not actual buffer
    let products = user.cart; // contains [{product, quantity}]
    let success = req.flash('success');
    //after add address route

    let addresses = user.addresses;
    // console.log(success);
    res.render("cart", {products, success,addresses });
});
...
router.get('/remove/:productid', isLoggedIn, async function (req,res) {
    ...
    await UserActivation.updateMany(
        {},
        {$pull :  {"cart.items" : {productId :  req.params.productid}}}
    )
    await user.save();
...
router.post('/add-address',isLoggedIn, async function(req,res){
    ...
    let isDefault = req.user.addresses.length === 0;
...
router.post('/set-default-address',isLoggedIn,async function(req,res){
    try{
        let user = await userModel.findOne({_id : req.user._id});
        let defaultAddress = req.body.selectedAddress;
    
        //tp undo prev default
        user.addresses.forEach(addr => addr.isDefault = false);
        let found = user.addresses.id(defaultAddress);
        if(found) found.isDefault = true;
        user.save().then(req.flash("success", "Changed the defualt address"));
        res.redirect('/cart');
    }catch(err){
...
router.get('/delete-address/:addressid',isLoggedIn,async function(req,res){
    try{
        let user = await userModel.findOne({_id : req.user._id});
        let selectedAdd = user.addresses.id(req.params.addressid);
        // res.send(selectedAdd);
        selectedAdd.deleteOne();
        user.save();
        req.flash("success", "Removed address")
...
router.get('/req',isLoggedIn, async function(req,res){
    let user = await userModel.findOne({_id : req.user._id});
    res.send(user);
})
