...
router.post('/login', validate(ownerLoginSchema),async function(req,res){
    ...
    try{
        ...
        res.cookie("token", token, {
            ...
            sameSite: "lax", // allows redirects to keep cookie
        });
...
router.get("/admin",isOwner,isAdmin,function(req,res){
    let success = req.flash('success',"product added");
    res.render("createproducts", {success});
})

router.post('/admin',isOwner,isAdmin, function(req,res){

    const success = req.flash('success', "product added");
    res.render("createproducts", { success });
})
//protected route
router.get("/allproducts",isOwner,isAdmin,async function(req,res){
